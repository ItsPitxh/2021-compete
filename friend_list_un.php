...
<head>
    ...
    <title>บุคคลที่ยังไม่ได้เป็นเพื่อน | Lannapoly</title>
...
<div class="shadow-sm p-3 w-3 bg-white">
    ...
    <hr>
<?php
$sql = "SELECT * FROM user_tb WHERE NOT user_tb.user_id IN (SELECT user_id_one FROM friend_tb WHERE user_id_two='$user_id' UNION SELECT user_id_two FROM friend_tb WHERE user_id_one='$user_id') AND user_status!='regis' ORDER BY user_fname asc";
...
?>
        <div class="shadow-none p-3 w-3" style="background-color: #dedede;">
            ...
            <!-- Profile -->
                <div class="prof">
                    <a href="friend_profile.php?friend_id=<?=$row['user_id']?>" class="btn btn-primary">ดูโปรไฟล์</a>
                </div>
            <!-- ----- -->
            <br>
        </div>
