...
<head>
    ...
    <title>เพื่อนทั้งหมด | Lannapoly</title>
...
<div class="shadow-sm p-3 w-3 bg-white">
    ...
    <hr>
<?php
$sql = "SELECT * FROM user_tb INNER JOIN friend_tb ON user_tb.user_id=friend_tb.user_id_one OR user_tb.user_id=friend_tb.user_id_two WHERE (user_id_one='$user_id' OR user_id_two='$user_id') AND friend_status='friend' AND user_status!='regis'";
...
?>
        <div class="shadow-none p-3 w-3" style="background-color: #dedede;">
            ...
            <!-- Profile -->
                <div class="prof">
                    <a href="friend_profile.php?friend_id=<?=$row['user_id']?>" class="btn btn-info">เพื่อน</a>
                </div>
            <!-- ----- -->
            <br>
        </div>
